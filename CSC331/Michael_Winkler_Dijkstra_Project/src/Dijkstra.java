...
public class Dijkstra {
	...
	private ArrayList<DijkstraVertex> finalPath;
	...
	public void findPath(Vertex i, Vertex j) {
		...
		while (priorityQueue.size() > 0) {
			...
			//Find the node in priorityQueue with the smallest distance
			//and make it the new expanded node
		...
		//Put all the parents of g into finalPath
		finalPath.add(g);
		while(parents.get(g) != null){
			finalPath.add(parents.get(g));
			g = parents.get(g);
		}
		
		//Then iterate backwards through finalPath so that the parents print in the 
		//correct order. 
		for (int z = finalPath.size() -1; z>=0; z--){
			System.out.print(finalPath.get(z).getName());
			if(z!=0)
				System.out.print("::");
		}
