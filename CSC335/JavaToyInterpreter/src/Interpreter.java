...
public class Interpreter {
	...
	public Object convertNameToInstance(String name){
		if (!(mySymbolTable.get(name) == null)){
	...
	public Object[] convertNameToInstance(String[] names){
		...
		for (int i=0; i<names.length; i++){
			newObjects[i] = convertNameToInstance(names[i]);
		}
		return newObjects;
	}
	...
	public String makeObject(ParseResults parse){	
		...
		Object object = ReflectionUtilities.createInstance(parse.className, arguments);
		mySymbolTable.put(parse.objectName, object);
		return "Okay. I have a new " + parse.className + " called " + parse.objectName; 
	}
	...
	public String callMethod(ParseResults parse){
		...
		Object target = convertNameToInstance(parse.objectName);
